...
function school_showlist(aPageNo,items){ 
	...
	for(h in array_school_showing){
		html+='<li  stationtelecode="'+array_school_showing[h].stationTelecode+'" item_index="'+h+'"><a href="#'+h+'">'+array_school_showing[h].chineseName+'</a></li>';
	}
	if (html == '') {
...
(function($) {
	/*function school_suggest(input, school_options) {
	    ...
	    // var framestr='<iframe src="" style="border:none;position:absolute; visibility:inherit; top:0px; left:0px; width:100%; height:100%; z-index:-1; filter=progid:DXImageTransform.Microsoft.Alpha(style=0,opacity=0);"></iframe>';
			var school_timeout = false;		// hold timeout ID for suggestion results to appear	
			var school_prevLength = 0;			// last recorded length of $input.val()
