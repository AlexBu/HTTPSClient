...
void CCheckPage::BuildRequest( TicketInfo& input )
{
	...
	reqData.Format(L"org.apache.struts.taglib.html.TOKEN=%s"
		...
		L"&orderRequest.seat_detail_type_code="
		...
		//L"&passenger_2_seat_detail_select=0"
		L"&passenger_2_seat_detail=%s"
		...
		,seat_detailGet(input.passengers[0])
		...
		,seat_detailGet(input.passengers[1])
...
{
	CString str(L"");
	str.Format(L"%s%%2C%s%%2C%s%%2C%s%%2C%s%%2C%s%%2C%s%%2CN",
		passenger.seat,
		seat_detailGet(passenger),
		ticketGet(passenger),
...
CString CCheckPage::seat_detailGet( PassengerInfo& passenger )
{
	return L"0";
}
